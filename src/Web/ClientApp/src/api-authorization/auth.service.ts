...
export class AuthService {
  ...
  initialize(): Observable<boolean> {
    return this.usersClient.infoGET().pipe(
      tap((res: any) => this._userName.next(res?.email || res?.userName || 'User')),
      map(() => true),
      catchError(() => of(false)),
  ...
  login(email: string, password: string): Observable<void> {
    return this.usersClient.login(true, undefined, new LoginRequest({ email, password })).pipe(
      ...
      tap(() => this.usersClient.infoGET().subscribe((res: any) => this._userName.next(res?.email || 'User'))),
      map(() => void 0)
    );
  }
